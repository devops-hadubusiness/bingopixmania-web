...
function Login() {
  const [selectedTab, setSelectedTab] = useState<AuthContextProps>('LOGIN')
  ...
  return (
    <div className="h-screen w-full flex-1 flex flex-row-reverse">
      ...
      <div className="lgAndDown:w-full xl:w-1/2 xlAndUp:w-1/3 h-full p-20 overflow-y-auto relative flex flex-col bg-gray-850">
        <div className="flex md:justify-center lg:justify-center gap-x-2">
          <img src="/images/logos/logo.svg" width={300} height={26} alt="Bingo Pix Mania" className="-mt-24" />
        </div>

        <Tabs value={selectedTab} defaultValue={selectedTab} className="w-full">
          <TabsList className="bg-primary/75 rounded-full">
            <TabsTrigger value="LOGIN" onClick={() => setSelectedTab('LOGIN')} className="rounded-full">
              Login
            </TabsTrigger>

            <TabsTrigger value="SIGNUP" onClick={() => setSelectedTab('SIGNUP')} className="rounded-full">
              Cadastro
            </TabsTrigger>
          </TabsList>
