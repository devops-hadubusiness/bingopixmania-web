...
export function HomeNextGameSection({ parentLoading, nextGame }: HomeNextGameSectionProps) {
  return (
    <>
      <div className="flex w-full justify-between items-center border border-2 border-primary-text rounded-md py-2 relative">
        ...
        <div className="flex flex-col items-center justify-center gap-1 flex-grow">
          ...
          <span className={cn('font-semibold text-background text-sm', (parentLoading || !nextGame?.id) && 'skeleton min-w-[50px] h-5')}>{String(nextGame?.id).padStart(6, '0')}</span>
        </div>
        ...
        <div className="flex flex-col items-center justify-center border-x border-primary-text gap-1 flex-grow">
          ...
          <span className={cn('font-semibold text-background text-sm', (parentLoading || !nextGame?.ticketPrice) && 'skeleton min-w-[50px] h-5')}>{formatBRL(nextGame?.ticketPrice as number)}</span>
        </div>

        <div className="flex flex-col items-center justify-center gap-1 flex-grow">
          <span className="font-semibold text-primary-text text-md">Hora</span>
          <span className={cn('font-semibold text-background text-sm', (parentLoading || !nextGame?.dateTime) && 'skeleton min-w-[50px] h-5')}>{nextGame?.dateTime ? formatTimestampToPattern(nextGame?.dateTime, 'dd/MM HH:mm') : ''} </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-y-2 w-full">
        <div className="flex w-full justify-between items-center bg-primary rounded-md p-3">
          ...
          <span className={cn('text-xl text-primary-foreground font-bold', (parentLoading || !nextGame?.firstPrizeValue) && 'skeleton min-w-[50px] h-8')}>{formatBRL(nextGame?.firstPrizeValue as number)}</span>
        </div>
        ...
        <div className="flex w-full justify-between items-center bg-primary rounded-md p-3">
          ...
          <span className={cn('text-xl text-primary-foreground font-bold', (parentLoading || !nextGame?.secondPrizeValue) && 'skeleton min-w-[50px] h-8')}>{formatBRL(nextGame?.secondPrizeValue as number)}</span>
        </div>
        ...
        <div className="flex w-full justify-between items-center bg-primary rounded-md p-3 smalltobig">
          ...
          <span className={cn('text-xl text-primary-foreground font-bold', (parentLoading || !nextGame?.thirdPrizeValue) && 'skeleton min-w-[50px] h-8')}>{formatBRL(nextGame?.thirdPrizeValue as number)}</span>
        </div>
      </div>
    </>
  )
